...
export function planLabel(id, plans) {
  ...
  return p ? p.nome : id; // Usar 'nome' ao invés de 'name'
}
...
// Cálculo com desconto (tela) - AGORA USA discAdPct e discMenPct do config
...
// Cálculo de fechamento (desconto extra na adesão) - AGORA USA discClosePct do config
...
export function parseIFPlan(text, plans) {
  const t = text.toLowerCase();
  for (let p of plans) {
    if (t.includes(p.nome.toLowerCase()) || t.includes(p.id)) return p.id; // Usar 'nome'
  }
  if (/
\b
top
