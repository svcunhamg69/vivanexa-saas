...
export function calcQuoteWithDiscount(mods, planId, ifPlan, cnpjs, notas, config = {}) {
  // Pega os percentuais de desconto das configurações
  const { discAdPct = 50, discMenPct = 0, unlimitedStrategy = true, prices = {}, plans = [], productNames = {} } = config;
  const results = [];
  let tAd = 0, tMen = 0, tAdD = 0, tMenD = 0; // tAdD e tMenD são os valores antes do desconto
  for (let mod of mods) {
    if (mod === 'IF') {
      const p = ifPlan || 'basic';
      const [aB, mB] = getPriceForPlan('IF', p, prices, plans);
      const ad = aB * 2, men = mB * 1.2;
      const adD = aB, menD = mB; // Valores base para IF
    ...
    if (mod === 'EP') {
      const epPlan = planId === 'topplus' ? 'top' : planId;
      const [, mB] = getPriceForPlan('EP', epPlan, prices, plans);
      const men = mB * 1.2, menD = mB; // Valores base para EP
    ...
    if (mod === 'XML') men = Math.max(men, 175);
    if (mod === 'Gestão Fiscal') men = Math.max(men, 200);
    const adD = aB > 0 ? aB : 0; // Valor base de adesão
    const menD = mB; // Valor base de mensalidade
  ...
  results.forEach(r => {
    if (!r.isTributos && !r.isEP) { // Tributos e EP não têm desconto de adesão
      r.ad = r.ad * (1 - discAd);
      r.adD = r.adD * (1 - discAd);
    }
    if (!r.isTributos && !r.isEP && !r.isIF) { // IF, Tributos e EP não têm desconto de mensalidade
...
export function calcClosing(mods, planId, ifPlan, cnpjs, notas, config = {}) {
  // Pega o percentual de desconto de fechamento das configurações
  const { discClosePct = 40, unlimitedStrategy = true, prices = {}, plans = [], productNames = {} } = config;
  const results = [];
  let tAd = 0, tMen = 0;
  const cp = discClosePct / 100; // Percentual de desconto de fechamento
  for (let mod of mods) {
    if (mod === 'IF') {
      const p = ifPlan || 'basic';
      const [aB, mB] = getPriceForPlan('IF', p, prices, plans);
      const ad = aB * (1 - cp); // Aplica desconto de fechamento na adesão do IF
    ...
    const ad = aB > 0 ? Math.max(aB * (1 - cp), 0) : 0; // Aplica desconto de fechamento na adesão
    let men = 0;
...
export function parseModules(text, productNames) {
  ...
  if (/gest[aã]o\s*(e\s*an[aá]lise|fiscal)/i.test(tNoIF) || (/
\b
fiscal
\b
/i.test(tNoIF) && !/intelig/i.test(tNoIF)) || (gfName && tNoIF.includes(gfName))) found.push('Gestão Fiscal');
  if (/
\b
bia
\b
/i.test(t)) found.push('BIA');
  if (/
\b
cnd
\b
/i.test(t)) found.push('CND');
  if (/
\b
xml
\b
/i.test(t)) found.push('XML');
