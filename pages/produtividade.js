// pages/produtividade.js — Produtividade Vivanexa v3
// Consulta Tributária: NCM/EAN/Descrição + filtros por UF, Regime e Tipo de Negócio
// APIs: BrasilAPI (NCM) + Bluesoft Cosmos (EAN) + IBPT (alíquotas por estado)
...
// ─── Alíquotas ICMS padrão por UF (interno/intrasestadual) ───────────────────
// Fonte: tabelas CONFAZ vigentes — usar como fallback quando IBPT não responder
const ICMS_UF = {
  AC:18, AL:18, AM:20, AP:18, BA:20.5, CE:20, DF:20, ES:17,
  GO:17, MA:22, MG:18, MS:17, MT:17, PA:19, PB:20, PE:20.5,
  PI:21, PR:19.5, RJ:20, RN:20, RO:17.5, RR:20, RS:17,
  SC:17, SE:19, SP:18, TO:20,
}

// ─── Base local NCM (produtos mais consultados) ───────────────────────────────
const NCM_BASE = {
  '84713012': { descricao:'Notebook / Laptop',        ncm:'8471.30.12', ipi:0,  pis_cumulativo:0.65, cofins_cumulativo:3,   pis_nao_cum:1.65, cofins_nao_cum:7.6,  cest:null,       st_estados:['SP','RJ','MG','RS'],  obs:'Equipamentos de informática com IPI reduzido a 0%. Verificar ST por estado.' },
  '22021000': { descricao:'Água mineral sem gás',     ncm:'2202.10.00', ipi:0,  pis_cumulativo:0.65, cofins_cumulativo:3,   pis_nao_cum:1.65, cofins_nao_cum:7.6,  cest:'03.001.00', st_estados:['todos'],              obs:'Sujeito à ST na maioria dos estados. CEST 03.001.00.' },
  '62034200': { descricao:'Calça masculina algodão',  ncm:'6203.42.00', ipi:0,  pis_cumulativo:0.65, cofins_cumulativo:3,   pis_nao_cum:1.65, cofins_nao_cum:7.6,  cest:null,       st_estados:[],                     obs:'Vestuário isento de IPI. ST não habitual para confecções.' },
  '85171210': { descricao:'Smartphone',               ncm:'8517.12.10', ipi:8,  pis_cumulativo:0.65, cofins_cumulativo:3,   pis_nao_cum:1.65, cofins_nao_cum:7.6,  cest:'21.059.00', st_estados:['todos'],              obs:'Celulares com ST em praticamente todos os estados.' },
  '10063021': { descricao:'Arroz beneficiado',        ncm:'1006.30.21', ipi:0,  pis_cumulativo:0.065,cofins_cumulativo:0.3,  pis_nao_cum:0.065,cofins_nao_cum:0.3,  cest:'13.001.00', st_estados:[],                     obs:'Cesta básica. PIS/COFINS alíquota zero (Lei 10.925/2004).' },
  '04021010': { descricao:'Leite em pó integral',     ncm:'0402.10.10', ipi:0,  pis_cumulativo:0,    cofins_cumulativo:0,   pis_nao_cum:0,    cofins_nao_cum:0,    cest:null,       st_estados:[],                     obs:'Isenção total de PIS/COFINS para leite. ICMS pode ser isento em alguns estados.' },
  '30049099': { descricao:'Medicamentos em geral',    ncm:'3004.90.99', ipi:0,  pis_cumulativo:0,    cofins_cumulativo:0,   pis_nao_cum:0,    cofins_nao_cum:0,    cest:'13.023.00', st_estados:['todos'],              obs:'Medicamentos geralmente isentos de IPI e PIS/COFINS. Verificar lista positiva/negativa.' },
  '87032310': { descricao:'Automóvel passageiros',    ncm:'8703.23.10', ipi:13, pis_cumulativo:0.65, cofins_cumulativo:3,   pis_nao_cum:1.65, cofins_nao_cum:7.6,  cest:'22.001.00', st_estados:['todos'],              obs:'Veículos com ST para ICMS. IPI varia por cilindrada e tipo de combustível.' },
}

// ─── Regras de ICMS por tipo de negócio ──────────────────────────────────────
function getIcmsInfo(uf, regime, tipoNegocio, icmsBase) {
  const aliq = ICMS_UF[uf] || 18
  let reducao = 0
  let obs = []

  if (tipoNegocio === 'industria') {
    obs.push('Operações de saída da indústria podem ter crédito de ICMS nas entradas.')
    if (regime === 'simples') { reducao = 0; obs.push('No Simples Nacional, o ICMS está incluído na alíquota unificada do DAS.') }
  }
  if (tipoNegocio === 'atacadista') {
    obs.push('Atacadistas podem ter diferimento ou redução de BC em alguns estados.')
  }
  if (tipoNegocio === 'varejo' && regime === 'simples') {
    obs.push('No Simples Nacional, o ICMS está incluído na alíquota do DAS, sem destaque na NF-e (exceto ST).')
  }
  if (tipoNegocio === 'importador') {
    obs.push(`ICMS na importação: ${aliq}% sobre (valor aduaneiro + II + IPI + PIS + COFINS + outras despesas).`)
  }

  return { aliquota: aliq - reducao, obs }
}

// ─── PIS/COFINS por regime ───────────────────────────────────────────────────
function getPisCofins(regime, tipoNegocio, produtoBase) {
  if (regime === 'simples' || regime === 'mei') {
    return { pis: 0, cofins: 0, obs: 'No Simples Nacional/MEI, PIS e COFINS estão incluídos no DAS.' }
  }
  if (regime === 'real' || tipoNegocio === 'industria') {
    return {
      pis: produtoBase?.pis_nao_cum ?? 1.65,
      cofins: produtoBase?.cofins_nao_cum ?? 7.6,
      obs: 'Regime não cumulativo (Leis 10.637/2002 e 10.833/2003). Permite aproveitamento de créditos.',
    }
  }
  // Presumido → regime cumulativo
  return {
    pis: produtoBase?.pis_cumulativo ?? 0.65,
    cofins: produtoBase?.cofins_cumulativo ?? 3,
    obs: 'Regime cumulativo (Lei 9.718/1998). Não permite aproveitamento de créditos.',
  }
}

// ─── Verificação de Substituição Tributária ───────────────────────────────────
function verificarST(uf, produtoBase) {
  if (!produtoBase?.st_estados) return { temST: false }
  const temST = produtoBase.st_estados.includes('todos') || produtoBase.st_estados.includes(uf)
  return {
    temST,
    cest: produtoBase.cest,
    obs: temST
      ? `Este produto está sujeito à Substituição Tributária em ${uf}. Retenção do ICMS-ST na origem.`
      : `Não identificada ST para este produto em ${uf}.`,
  }
}
...
async function buscarNCMDescricao(descricao) {
  try {
    const r = await fetch(`https://brasilapi.com.br/api/ncm/v1?search=${encodeURIComponent(descricao)}`)
    if (!r.ok) return null
...
// ─── Busca IBPT — alíquotas aproximadas por NCM e UF ────────────────────────
// A API pública do IBPT exige token (cadastro gratuito). Aqui usamos o endpoint
// público de consulta simplificada disponível em: https://deolhnoimposto.ibpt.com.br
async function buscarIBPT(ncm, uf) {
  try {
    const clean = ncm.replace(/\D/g, '')
    // Endpoint público IBPT (sem token necessário para consulta básica)
    const r = await fetch(
      `https://apidoni.ibpt.org.br/api/v1/produtos?token=demo&codigo=${clean}&uf=${uf}&ex=0`,
...
export default function Produtividade() {
  ...
  // Consulta Tributária — campos de busca
  ...
  // Lista de NCMs quando busca por descrição retornar vários
  ...
  // ── CONSULTA TRIBUTÁRIA ──────────────────────────────────────────────────────
  function montarResultadoCompleto({ ncmCodigo, descricao, ipi, produtoBase, ean, fonte }) {
    const icmsInfo   = getIcmsInfo(tribUF, tribRegime, tribNegocio, null)
    const pisCofins  = getPisCofins(tribRegime, tribNegocio, produtoBase)
    const stInfo     = verificarST(tribUF, produtoBase)
    ...
    const ipiEfetivo = tribNegocio === 'varejo' || tribNegocio === 'atacadista' ? (ipi ?? produtoBase?.ipi ?? null) : (ipi ?? produtoBase?.ipi ?? null)
    ...
    return {
      ...
      ean: ean || null,
      ipi: ipiEfetivo,
      icms: icmsInfo.aliquota,
      pis: pisCofins.pis,
      cofins: pisCofins.cofins,
      cest: stInfo.cest || produtoBase?.cest || null,
      temST: stInfo.temST,
      uf: tribUF,
      regime: tribRegime,
      tipoNegocio: tribNegocio,
      observacoes,
      fonte,
      lei_ipi: ipiEfetivo > 0
        ? 'TIPI – Tabela de Incidência do IPI (Decreto 11.158/2022)'
        : 'TIPI – Posição isenta ou com alíquota 0%',
      lei_icms: `RICMS/${tribUF} – Alíquota interna ${icmsInfo.aliquota}% (base CONFAZ). Verificar convênios e protocolos vigentes.`,
      lei_pis_cofins: tribRegime === 'real' || tribNegocio === 'industria'
        ? 'Regime não cumulativo – Leis 10.637/2002 (PIS) e 10.833/2003 (COFINS)'
        : tribRegime === 'simples' || tribRegime === 'mei'
          ? 'PIS/COFINS incluídos no DAS – LC 123/2006'
          : 'Regime cumulativo – Lei 9.718/1998',
    }
  }

  async function consultarTributaria() {
    if (!tribBusca.trim()) {
      setTribErro(`Informe o ${tribTipo === 'ncm' ? 'NCM' : tribTipo === 'ean' ? 'código de barras' : 'descrição'}`)
      return
    }
    setTribLoading(true); setTribErro(''); setTribResult(null); setTribLista([])

    try {
      const termo = tribBusca.trim().replace(/[.\-\/\s]/g, '')

      // ────────────── BUSCA POR NCM ──────────────────────────────────────────
      if (tribTipo === 'ncm') {
        const produtoBase = NCM_BASE[termo] || NCM_BASE[tribBusca.trim()]

        // 1. Tenta IBPT primeiro (dados reais por UF)
        const ibpt = await buscarIBPT(termo, tribUF)
        if (ibpt && (ibpt.nacional !== undefined || ibpt.estadual !== undefined)) {
          const resultado = montarResultadoCompleto({
            ncmCodigo: tribBusca.trim(),
            descricao: ibpt.descricao || produtoBase?.descricao || 'Produto',
            ipi: ibpt.importadosFederal ?? produtoBase?.ipi ?? 0,
            produtoBase,
            fonte: 'ibpt',
          })
          resultado.ibpt = { nacional: ibpt.nacional, estadual: ibpt.estadual, municipal: ibpt.municipal }
          setTribResult(resultado)
          setTribLoading(false)
          return
        }

        // 2. Fallback: BrasilAPI (dados básicos)
        const api = await buscarNCMApi(termo)
        if (api && api.codigo) {
          const resultado = montarResultadoCompleto({
            ncmCodigo: api.codigo,
            descricao: api.descricao,
            ipi: api.aliquota_ipi ?? produtoBase?.ipi ?? 0,
            produtoBase,
            fonte: 'brasilapi',
          })
          setTribResult(resultado)
        ...
        // 3. Fallback: base local
        if (produtoBase) {
          setTribResult(montarResultadoCompleto({
            ncmCodigo: produtoBase.ncm,
        ...
        setTribErro('NCM não encontrado. Verifique o código ou tente buscar por descrição.')

      // ────────────── BUSCA POR EAN ──────────────────────────────────────────
      } else if (tribTipo === 'ean') {
        const eanData = await buscarEAN(termo)
        if (eanData && (eanData.description || eanData.descricao)) {
          const ncmCode = eanData.ncm?.code || eanData.ncm || null
          const ncmClean = ncmCode ? String(ncmCode).replace(/\D/g, '') : null
          const produtoBase = ncmClean ? (NCM_BASE[ncmClean] || null) : null

          // Se tem NCM, busca alíquotas por UF
          let ibpt = null
          if (ncmClean) ibpt = await buscarIBPT(ncmClean, tribUF)

          const resultado = montarResultadoCompleto({
            ncmCodigo: ncmCode ? String(ncmCode) : 'Não disponível',
            descricao: eanData.description || eanData.descricao,
            ipi: ibpt?.importadosFederal ?? produtoBase?.ipi ?? null,
            produtoBase,
            ean: termo,
            fonte: 'ean',
          })
          if (ibpt) resultado.ibpt = { nacional: ibpt.nacional, estadual: ibpt.estadual, municipal: ibpt.municipal }
          setTribResult(resultado)
        } else {
          setTribErro('Produto não encontrado pelo código de barras. Tente pelo NCM.')
        }

      // ────────────── BUSCA POR DESCRIÇÃO ───────────────────────────────────
      } else {
        // 1. Busca na BrasilAPI
        const lista = await buscarNCMDescricao(tribBusca.trim())
        if (lista && lista.length > 0) {
          if (lista.length === 1) {
            // Resultado único — resolve direto
            const item = lista[0]
            const ncmClean = item.codigo.replace(/\D/g,'')
            const produtoBase = NCM_BASE[ncmClean] || null
            const resultado = montarResultadoCompleto({
              ncmCodigo: item.codigo,
              descricao: item.descricao,
              ipi: produtoBase?.ipi ?? 0,
              produtoBase,
              fonte: 'brasilapi',
            })
            setTribResult(resultado)
  ...
  // Seleciona um item da lista de NCMs (quando busca por descrição retorna vários)
  async function selecionarNCM(item) {
    setTribLista([])
    setTribLoading(true)
    const ncmClean = item.codigo.replace(/\D/g,'')
    const produtoBase = NCM_BASE[ncmClean] || null
    const ibpt = await buscarIBPT(ncmClean, tribUF)
    const resultado = montarResultadoCompleto({
      ncmCodigo: item.codigo,
      descricao: item.descricao,
      ipi: ibpt?.importadosFederal ?? produtoBase?.ipi ?? 0,
      produtoBase,
      fonte: ibpt ? 'ibpt' : 'brasilapi',
    })
    if (ibpt) resultado.ibpt = { nacional: ibpt.nacional, estadual: ibpt.estadual, municipal: ibpt.municipal }
    setTribResult(resultado)
    setTribLoading(false)
  ...
  // Recalcula resultado quando UF/Regime/Tipo mudam (se já existe resultado)
  useEffect(() => {
    if (!tribResult) return
    const ncmClean = String(tribResult.ncm || '').replace(/\D/g,'')
    const produtoBase = NCM_BASE[ncmClean] || null
    const novo = montarResultadoCompleto({
      ncmCodigo: tribResult.ncm,
      descricao: tribResult.descricao,
      ipi: tribResult.ipi,
      produtoBase,
      ean: tribResult.ean,
      fonte: tribResult.fonte,
    })
    if (tribResult.ibpt) novo.ibpt = tribResult.ibpt
    setTribResult(novo)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tribUF, tribRegime, tribNegocio])
      ...
      <div className="page-wrap">
          ...
          <div>
            ...
            <div className="card" style={{marginBottom:20}}>
              ...
              <p style={{fontSize:12,color:'#64748b',marginBottom:18,lineHeight:1.6}}>
                Consulte IPI, ICMS, PIS, COFINS, CEST e Substituição Tributária. Informe o <strong style={{color:'#e2e8f0'}}>Estado</strong>, o <strong style={{color:'#e2e8f0'}}>Regime Tributário</strong> e o <strong style={{color:'#e2e8f0'}}>Tipo de Negócio</strong> para resultados personalizados.
              </p>

              {/* ── Linha 1: UF + Regime + Tipo de Negócio ── */}
              ...
              {/* ── Linha 2: Tipo de busca ── */}
              <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>
                {[
                  { id:'ncm',       label:'🏷️ NCM'             },
                  { id:'ean',       label:'📦 Código de Barras' },
                  { id:'descricao', label:'🔤 Descrição'        },
                ].map(t => (
                  <button key={t.id}
                    onClick={() => { setTribTipo(t.id); setTribBusca(''); setTribResult(null); setTribErro(''); setTribLista([]) }}
                    style={{
                      padding:'7px 16px', borderRadius:8, border:'1px solid', cursor:'pointer',
                      fontFamily:'DM Mono,monospace', fontSize:12, fontWeight:600, transition:'all .14s',
              ...
              <div style={{display:'flex',gap:10}}>
                <input
                  ...
                  placeholder={
                    tribTipo === 'ncm'       ? 'Digite o código NCM (ex: 8471.30.12)' :
                    tribTipo === 'ean'       ? 'Digite o código de barras EAN'         :
                ...
                <button onClick={consultarTributaria} disabled={tribLoading}
                  style={{padding:'11px 24px',borderRadius:8,background:'linear-gradient(135deg,#00d4ff,#0099bb)',border:'none',color:'#fff',fontFamily:'DM Mono,monospace',fontSize:13,fontWeight:600,cursor:tribLoading?'not-allowed':'pointer',opacity:tribLoading?.7:1,flexShrink:0,whiteSpace:'nowrap'}}>
              ...
              <div style={{marginTop:14,display:'flex',gap:8,flexWrap:'wrap'}}>
                {[
                  {label:'BrasilAPI (NCM)',    cor:'#00d4ff'},
                  {label:'Bluesoft Cosmos (EAN)', cor:'#7c3aed'},
                  {label:'IBPT (Alíquotas)',   cor:'#10b981'},
                  {label:'CONFAZ (ICMS/ST)',   cor:'#f59e0b'},
                ].map(f => (
                  <span key={f.label} style={{fontSize:10,color:f.cor,background:f.cor+'15',border:`1px solid ${f.cor}33`,padding:'3px 9px',borderRadius:20,fontWeight:700}}>
                    {f.label}
                  </span>
                ))}
              </div>
            ...
            {/* ── Lista de resultados (busca por descrição com múltiplos) ── */}
            {tribLista.length > 0 && (
              <div className="card" style={{marginBottom:20}}>
                <div className="card-title">📋 Selecione o produto correto</div>
                <p style={{fontSize:12,color:'#64748b',marginBottom:14}}>Encontramos {tribLista.length} produtos. Clique para ver a tributação detalhada.</p>
                {tribLista.map(item => (
                  <div key={item.codigo}
                    onClick={() => selecionarNCM(item)}
                    style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'#1a2540',border:'1px solid #1e2d4a',borderRadius:9,marginBottom:6,cursor:'pointer',transition:'border-color .14s'}}
                    onMouseEnter={e => e.currentTarget.style.borderColor='rgba(0,212,255,.4)'}
                    onMouseLeave={e => e.currentTarget.style.borderColor='#1e2d4a'}
                  >
                    <span style={{background:'rgba(0,212,255,.12)',border:'1px solid rgba(0,212,255,.3)',color:'#00d4ff',padding:'3px 10px',borderRadius:20,fontSize:11,fontWeight:700,flexShrink:0}}>
                      NCM {item.codigo}
                    </span>
                    <span style={{fontSize:13,color:'#e2e8f0'}}>{item.descricao}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {tribResult && (
              <div className="card">
                ...
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10}}>
                  <div>
                    <div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:800,color:'#e2e8f0',marginBottom:8}}>{tribResult.descricao}</div>
                    <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                      ...
                      {tribResult.temST && <span style={{background:'rgba(239,68,68,.12)',border:'1px solid rgba(239,68,68,.3)',color:'#ef4444',padding:'3px 10px',borderRadius:20,fontSize:11,fontWeight:700}}>⚡ ST em {tribResult.uf}</span>}
                    </div>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:4,alignItems:'flex-end'}}>
                    <span style={{fontSize:10,color:'#64748b',background:'#1a2540',border:'1px solid #1e2d4a',padding:'4px 10px',borderRadius:6}}>
                      {tribResult.fonte === 'ibpt' ? '📡 IBPT' : tribResult.fonte === 'brasilapi' ? '🌐 BrasilAPI' : tribResult.fonte === 'ean' ? '📦 Cosmos/EAN' : '📚 Base local'}
                    </span>
                    <span style={{fontSize:10,color:'#64748b',background:'#1a2540',border:'1px solid #1e2d4a',padding:'4px 10px',borderRadius:6}}>
                      {tribResult.uf} · {regimeLbl} · {negocioLbl}
                    </span>
                  </div>
                </div>

                {/* IBPT — carga aproximada total */}
                {tribResult.ibpt && (
                  <div style={{background:'rgba(16,185,129,.06)',border:'1px solid rgba(16,185,129,.2)',borderRadius:10,padding:'14px 18px',marginBottom:18}}>
                    <div style={{fontSize:11,color:'#10b981',fontWeight:700,marginBottom:10,textTransform:'uppercase',letterSpacing:1}}>📊 Carga Tributária Aproximada — IBPT (De Olho no Imposto)</div>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                      {[
                        {l:'Federal',   v:tribResult.ibpt.nacional,  c:'#00d4ff'},
                        {l:'Estadual',  v:tribResult.ibpt.estadual,  c:'#f59e0b'},
                        {l:'Municipal', v:tribResult.ibpt.municipal, c:'#10b981'},
                      ].map(i => (
                        <div key={i.l} style={{background:'#111827',border:'1px solid #1e2d4a',borderRadius:8,padding:'10px 14px',textAlign:'center'}}>
                          <div style={{fontSize:9,color:'#64748b',textTransform:'uppercase',letterSpacing:1,marginBottom:4}}>{i.l}</div>
                          <div style={{fontFamily:'Syne,sans-serif',fontSize:22,fontWeight:800,color:i.c}}>{i.v !== undefined ? `${Number(i.v).toFixed(2)}%` : '—'}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{fontSize:10,color:'#64748b',marginTop:8}}>* Alíquotas aproximadas conforme Lei 12.741/2012 (Transparência Fiscal). Dados do IBPT para {tribResult.uf}.</div>
                  </div>
                )}

                {/* Grid de tributos principais */}
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
                  {[
                    {label:'IPI',    val:tribResult.ipi,    color:'#7c3aed', desc:'Imposto s/ Produtos Industrializados'},
                    {label:'ICMS',   val:tribResult.icms,   color:'#f59e0b', desc:`ICMS ${tribResult.uf} — alíquota interna`},
                    {label:'PIS',    val:tribResult.pis,    color:'#10b981', desc:'Programa de Integração Social'},
                    {label:'COFINS', val:tribResult.cofins, color:'#00d4ff', desc:'Contrib. Financ. Seguridade Social'},
                  ].map(t => (
                    <div key={t.label} style={{background:'#1a2540',border:`1px solid ${t.color}33`,borderRadius:10,padding:'14px 12px',textAlign:'center'}}>
                      ...
                      <div style={{fontFamily:'Syne,sans-serif',fontSize:26,fontWeight:800,color:t.val!==null?t.color:'#334155',marginBottom:4}}>
                        {t.val !== null ? `${Number(t.val).toFixed(2)}%` : tribResult.regime === 'simples' || tribResult.regime === 'mei' ? 'DAS' : 'Varia*'}
                      </div>
                ...
                <div style={{background:'rgba(0,212,255,.04)',border:'1px solid rgba(0,212,255,.15)',borderRadius:10,padding:'16px 18px',marginBottom:14}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                    {[
                      ...
                      ['📋 CEST/ST',    tribResult.cest
                        ? `CEST ${tribResult.cest} — Convênio ICMS/Protocolos CONFAZ. ${tribResult.temST ? `ST aplicável em ${tribResult.uf}.` : ''}`
                        : 'Não sujeito à Substituição Tributária (verificar protocolos por estado).'],
                    ].map(([l, v]) => (
                ...
                {/* Observações */}
                {tribResult.observacoes?.length > 0 && (
                  <div style={{background:'rgba(245,158,11,.06)',border:'1px solid rgba(245,158,11,.2)',borderRadius:8,padding:'12px 14px',marginBottom:14}}>
                    <div style={{fontSize:10,color:'#f59e0b',fontWeight:700,marginBottom:8}}>⚠️ OBSERVAÇÕES</div>
                    {tribResult.observacoes.map((obs, i) => (
                      <div key={i} style={{fontSize:12,color:'#94a3b8',lineHeight:1.6,marginBottom:i < tribResult.observacoes.length-1 ? 6 : 0}}>
                        • {obs}
                      </div>
                    ))}
                  </div>
                ...
                {/* Carga total + simulação */}
                <div style={{marginTop:14,display:'flex',gap:10,flexWrap:'wrap'}}>
                  <div style={{flex:1,minWidth:140,background:'#1a2540',border:'1px solid #1e2d4a',borderRadius:10,padding:'14px 16px',textAlign:'center'}}>
                    <div style={{fontSize:10,color:'#64748b',marginBottom:4}}>CARGA TRIBUTÁRIA TOTAL</div>
                    <div style={{fontFamily:'Syne,sans-serif',fontSize:28,fontWeight:800,color:'#ef4444'}}>{cargaTotal}%</div>
                    <div style={{fontSize:10,color:'#64748b',marginTop:2}}>{tribResult.uf} · {regimeLbl}</div>
                  </div>
                  <div style={{flex:2,minWidth:220,background:'#1a2540',border:'1px solid #1e2d4a',borderRadius:10,padding:'14px 16px'}}>
                    <div style={{fontSize:10,color:'#64748b',marginBottom:10}}>SIMULAÇÃO — PRODUTO DE R$ 100,00</div>
                    {[
                      ['IPI',    tribResult.ipi],
                    ...
                    <div style={{borderTop:'1px solid #1e2d4a',paddingTop:6,marginTop:6,display:'flex',justifyContent:'space-between',fontSize:13,fontWeight:700}}>
                      <span style={{color:'#64748b'}}>Total tributos:</span>
                      <span style={{color:'#ef4444'}}>R$ {cargaTotal}</span>
                    </div>
                  </div>
                </div>

                {/* Aviso legal */}
                <div style={{marginTop:14,padding:'10px 14px',background:'rgba(100,116,139,.06)',border:'1px solid rgba(100,116,139,.2)',borderRadius:8,fontSize:11,color:'#64748b',lineHeight:1.6}}>
                  ℹ️ <strong>Importante:</strong> As alíquotas são aproximadas e baseadas nas fontes BrasilAPI, IBPT e CONFAZ. Alíquotas de ICMS variam por estado, operação e convênios vigentes. Consulte sempre um contador para decisões fiscais.
                </div>
              </div>
            )}
          </div>
            ...
            {meis.length === 0 ? <div className="empty">Nenhum MEI cadastrado.</div> : meis.map(m => (
              <div key={m.id} className="list-row">
                ...
                <button className="act-btn del" onClick={async () => {
                  ...
                  const l = meis.filter(x => x.id !== m.id)
                  const nc = {...cfg, meis:l}
                  await salvarStorage(nc); setMeis(l); setCfg(nc); toast('🗑 Removido!')
                }}>🗑</button>
              </div>
            ))}
          </div>
        ...
        {aba === 'ecac' && (
          <div className="card">
            ...
            <p style={{fontSize:13,color:'#64748b',marginBottom:20,lineHeight:1.7}}>
              Integração com portais de monitoramento contábil e fiscal. Configure as credenciais em{' '}
              <strong style={{color:'#00d4ff',cursor:'pointer'}} onClick={() => router.push('/configuracoes?tab=integracoes')}>Configurações → Integrações</strong>.
            </p>
            <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:12}}>
              {[
                {nome:'VERI',             desc:'Monitoramento automático de CNDs, certidões e situação fiscal.',   cor:'#00d4ff'},
                {nome:'Monitor Contábil', desc:'Painel unificado de obrigações acessórias e prazos fiscais.',      cor:'#7c3aed'},
                {nome:'Fiscontech',       desc:'Gestão de malhas fiscais, pendências e regularização automática.', cor:'#10b981'},
                {nome:'SIEG',             desc:'Captura e organização automática de XMLs de NF-e e NFC-e.',       cor:'#f59e0b'},
              ].map(s => (
        ...
        {aba === 'notas' && (
          <div className="card">
            ...
            <p style={{fontSize:13,color:'#64748b',marginBottom:20,lineHeight:1.7}}>
              Integração com o Portal Nacional da NF-e para captura automática de XMLs. Configure em{' '}
              <strong style={{color:'#00d4ff',cursor:'pointer'}} onClick={() => router.push('/configuracoes?tab=integracoes')}>Configurações → Integrações</strong>.
            </p>
