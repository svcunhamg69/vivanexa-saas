...
// 1. Usa cfg.clients de configuracoes.js (com regime tributário adicional ao cliente)
// 2. CRUD completo de tarefas e obrigações (incluir, editar, excluir)
// 3. Replica obrigações para todos os meses do ano com datas corretas
// 4. Obrigações estaduais/municipais baseadas no estado/município do cliente
// 5. Prazos editáveis por obrigação (ideal, atenção, vencimento) + dashboard clicável
// 6. Atribuição de usuário por empresa (usuário ve só suas empresas, admin ve tudo)
// 7. Análise de eficiência por usuário
// 8. Filtro de pesquisa avançado
// 9. Obrigações vinculadas ao regime tributário, editáveis por regime
...
// dia_ideal = verde, dia_atencao = amarelo, dia_vencimento = vermelho
...
// Determina se uma obrigação ocorre num determinado mês/ano
function obgOcorreNoMes(obg, mes, ano) {
  if (obg.periodicidade === 'Mensal') return true
  if (obg.periodicidade === 'Trimestral') {
    // trimestres: mar(3), jun(6), set(9), dez(12)
...
// Gera obrigações para um cliente para todos os meses do ano
function gerarObrigacoesAnuais(cliente, obgExistentes, ano, customObgRegimes) {
  const regime = cliente.regime || 'simples'
  // Usa obrigações customizadas do regime se existirem, senão usa as base
    ...
    for (const obg of obgBase) {
      ...
      if (!jaExiste) {
        novas.push({
          ...
          clienteMun:  cliente.cidade || '',
          obgId:       obg.id,
          obgNome:     obg.nome,
          tipo:        obg.tipo,
          categoria:   obg.categoria,
          periodicidade: obg.periodicidade,
          dia_ideal:      obg.dia_ideal,
          dia_atencao:    obg.dia_atencao,
          dia_vencimento: obg.dia_vencimento,
          mes, ano,
          status:    'Pendente',
          dataConc:  null,
          obs:       '',
...
export default function Produtividade() {
  ...
  const [tribAba,     setTribAba]     = useState('pis')
  ...
  // ── Tarefas / Obrigações ─────────────────────────────────────────────────
  const [clientes,     setClientes]     = useState([])   // de cfg.clients (configuracoes)
  const [obrigacoes,   setObrigacoes]   = useState([])   // em cfg.contabilObrigacoes
  ...
  const [drillDown,      setDrillDown]      = useState(null) // 'pendentes'|'concluidas'|'atrasadas'|null
  ...
  const [customObgRegimes, setCustomObgRegimes] = useState({}) // obrigações customizadas por regime
  ...
  useEffect(() => {
    async function init() {
      ...
      if (row?.value) {
        ...
        setCfg(c)
        // ✅ Usa cfg.clients (cadastro de configuracoes) — filtra por usuário se não for admin
        const todosClientes = c.clients || []
        const isAdmin = perf?.perfil === 'admin' || perf?.perfil === 'gerente'
        const clientesFiltrados = isAdmin
          ? todosClientes
          : todosClientes.filter(cl => !cl.usuarioId || cl.usuarioId === perf.user_id)
  ...
  // ─── Gera/Replica obrigações para o ano inteiro ────────────────────────────
  async function gerarObrigacoesParaCliente(cliente, obgAtuais, customReg) {
    const novas = gerarObrigacoesAnuais(cliente, obgAtuais, anoRef, customReg)
    const merged = [...obgAtuais, ...novas]
    const novoCfg = { ...cfg, contabilObrigacoes: merged }
    await salvarStorage(novoCfg)
    setObrigacoes(merged)
    setCfg(novoCfg)
    return merged
  }

  // ─── Atribuir regime ao cliente (salva em cfg.clients) ────────────────────
  async function salvarRegimeCliente(clienteId, regime) {
    const novosClientes = (cfg.clients || []).map(c =>
      c.id === clienteId ? { ...c, regime } : c
    )
    const obgSemEsteCliente = obrigacoes.filter(o => o.clienteId !== clienteId)
    const clienteAtual = novosClientes.find(c => c.id === clienteId)
    const novasObg = clienteAtual
      ? gerarObrigacoesAnuais(clienteAtual, obgSemEsteCliente, anoRef, customObgRegimes)
      : []
    const merged = [...obgSemEsteCliente, ...novasObg]
    const novoCfg = { ...cfg, clients: novosClientes, contabilObrigacoes: merged }
    await salvarStorage(novoCfg)
    setCfg(novoCfg)
    setClientes(prev => prev.map(c => c.id === clienteId ? { ...c, regime } : c))
    setObrigacoes(merged)
    toast('✅ Regime atualizado! Obrigações geradas para o ano.')
  }

  // ─── Gerar obrigações para todos os clientes (replica para todos os meses) ──
  ...
  async function salvarObg() {
    ...
    let novas
    if (formObg.key) {
      novas = obrigacoes.map(o => o.key === formObg.key ? obgFinal : o)
    } else {
      novas = [...obrigacoes, obgFinal]
    }
  ...
  async function alterarStatusObg(key, novoStatus) {
    const novas = obrigacoes.map(o =>
      o.key === key
        ? { ...o, status: novoStatus, dataConc: novoStatus === 'Concluída' ? new Date().toISOString().slice(0,10) : null }
        : o
    )
    const novoCfg = { ...cfg, contabilObrigacoes: novas }
    await salvarStorage(novoCfg)
  ...
  // ─── Edição de prazos globais do regime ────────────────────────────────────
  async function salvarCustomObgRegime(regime, lista) {
    ...
    await salvarStorage(novoCfg)
    setCfg(novoCfg)
    setCustomObgRegimes(novoCustom)
    toast('✅ Obrigações do regime atualizadas!')
  }

  // ─── Atribuir usuário a cliente ───────────────────────────────────────────
  async function atribuirUsuario(clienteId, usuarioId) {
    const novosClients = (cfg.clients || []).map(c =>
      c.id === clienteId ? { ...c, usuarioId } : c
    )
    const novoCfg = { ...cfg, clients: novosClients }
    await salvarStorage(novoCfg)
    setCfg(novoCfg)
    setClientes(prev => prev.map(c => c.id === clienteId ? { ...c, usuarioId } : c))
    // Atualiza obrigações do cliente com novo usuário
    const novasObg = obrigacoes.map(o => o.clienteId === clienteId ? { ...o, usuarioId } : o)
    const novoCfg2 = { ...novoCfg, contabilObrigacoes: novasObg }
    await salvarStorage(novoCfg2)
    setObrigacoes(novasObg)
    setCfg(novoCfg2)
  ...
  const obgPendentes  = obgDoMes.filter(o => calcStatus(o) === 'Pendente' || calcStatus(o) === 'Em Andamento').length
  ...
  const obgFiltradas = obgDoMes.filter(o => {
    ...
    if (filtroStatus !== 'Todos') {
      if (calcStatus(o) !== filtroStatus) return false
    }
  ...
  function calcEficiencia() {
    ...
    obrigacoes.forEach(o => {
      const uid = o.usuarioId || 'sem_usuario'
      if (!stats[uid]) stats[uid] = { total:0, concluidas:0, atrasadas:0, noPrazo:0 }
      stats[uid].total++
      const st = calcStatus(o)
      if (st === 'Concluída') {
        stats[uid].concluidas++
        if (o.dataConc && o.dia_vencimento) {
          const dataConc = new Date(o.dataConc)
          const venc = new Date(o.ano, o.mes - 1, o.dia_vencimento)
          if (dataConc <= venc) stats[uid].noPrazo++
        }
      }
      if (st === 'Atrasada') stats[uid].atrasadas++
    })
    return stats
  }
  ...
  async function consultarTributaria(busca, tipo) {
    ...
    } catch { setTribErro('Erro de conexão.') }
    finally { setTribLoading(false) }
  }

  async function processarLote() {
    const linhas = loteTexto.trim().split('\n').map(l => l.trim()).filter(Boolean)
    if (!linhas.length) { setLoteErro('Cole os itens acima.'); return }
    ...
    } catch { setLoteErro('Erro ao processar lote.') }
  ...
  function handleFileUpload(e) {
    ...
    reader.onload = evt => { setLoteTexto(evt.target.result.split('\n').slice(0,200).join('\n')); toast('📄 Arquivo carregado!') }
      ...
      <div className="page-wrap">
        ...
        <div className="tabs">
          {[
            ...
            { id:'eficiencia',  label:'📈 Eficiência por Usuário' },
            { id:'regimes',     label:'⚙️ Configurar Regimes'     },
          ...
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
              {[
                ...
                <div key={m.l}
                  ...
                  style={{
                    background: drillDown === m.d ? `${m.c}18` : '#111827',
                    border: `1px solid ${drillDown === m.d ? m.c+'55' : '#1e2d4a'}`,
                    borderRadius:12, padding:'16px 18px', cursor: m.d ? 'pointer' : 'default',
                    transition:'all .2s'
                  }}>
            ...
            <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:14,flexWrap:'wrap'}}>
              <input
                ...
                className="field-input" style={{flex:'1 1 200px',minWidth:200,padding:'7px 12px',fontSize:12}}
              />
            ...
            {clientes.length === 0 ? (
              <div className="empty">
                ...
                <span style={{fontSize:12}}>Cadastre clientes em <strong style={{color:'#00d4ff'}}>Configurações → Clientes</strong> e defina o regime tributário abaixo.</span>
              </div>
              ...
              <div>
                ...
                {obgFiltradas.map(o => {
                  ...
                  const idealDia = o.dia_ideal ? new Date(o.ano, o.mes-1, o.dia_ideal) : null
            ...
            {/* Atribuição de regime e usuário por cliente */}
            {clientes.length > 0 && (
                ...
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  {clientes.map(c => (
                    <div key={c.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:'#111827',border:'1px solid #1e2d4a',borderRadius:10}}>
                      ...
                      <select
                        value={c.regime || 'simples'}
                        onChange={e => salvarRegimeCliente(c.id, e.target.value)}
                      ...
                      {isAdmin && (
                        <select
                          value={c.usuarioId || ''}
                          onChange={e => atribuirUsuario(c.id, e.target.value)}
                          style={{background:'#1a2540',border:'1px solid #1e2d4a',borderRadius:6,padding:'5px 10px',fontSize:12,color:'#e2e8f0',fontFamily:'DM Mono,monospace',cursor:'pointer'}}>
                          <option value="">Sem usuário</option>
                          {usuarios.map(u => <option key={u.id} value={u.id}>{u.nome || u.email}</option>)}
                        </select>
        ...
        {aba === 'eficiencia' && (
          <div>
            <div style={{marginBottom:20,fontSize:12,color:'#64748b'}}>
              Análise de performance por usuário responsável pelas obrigações.
            </div>
            {(() => {
              const stats = calcEficiencia()
              const entries = Object.entries(stats)
              if (!entries.length) return <div className="empty">Nenhum dado disponível. Atribua usuários às empresas.</div>
              return (
                <div style={{display:'flex',flexDirection:'column',gap:12}}>
                  {entries.map(([uid, s]) => {
                    const usuario = usuarios.find(u => u.id === uid)
                    const nome = usuario?.nome || usuario?.email || (uid === 'sem_usuario' ? 'Sem usuário' : uid)
                    const pct = s.total > 0 ? Math.round((s.concluidas / s.total) * 100) : 0
                    const noPrazoPct = s.concluidas > 0 ? Math.round((s.noPrazo / s.concluidas) * 100) : 0
                    return (
                      <div key={uid} style={{background:'#111827',border:'1px solid #1e2d4a',borderRadius:12,padding:'18px 20px'}}>
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
                          <div>
                            <div style={{fontSize:14,fontWeight:700,color:'#e2e8f0',marginBottom:2}}>{nome}</div>
                            <div style={{fontSize:11,color:'#64748b'}}>{s.total} obrigação(ões) no período</div>
                          </div>
                          <div style={{fontSize:28,fontWeight:800,fontFamily:'Syne,sans-serif',color:pct>=80?'#10b981':pct>=60?'#f59e0b':'#ef4444'}}>{pct}%</div>
                        </div>
                        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
                          {[
                            { l:'Total',      v:s.total,      c:'#00d4ff' },
                            { l:'Concluídas', v:s.concluidas, c:'#10b981' },
                            { l:'Atrasadas',  v:s.atrasadas,  c:'#ef4444' },
                            { l:'No Prazo',   v:`${noPrazoPct}%`, c:'#7c3aed' },
                          ].map(m => (
                            <div key={m.l} style={{background:'#1a2540',borderRadius:8,padding:'10px 12px',textAlign:'center'}}>
                              <div style={{fontSize:20,fontWeight:700,color:m.c,fontFamily:'Syne,sans-serif'}}>{m.v}</div>
                              <div style={{fontSize:10,color:'#64748b',marginTop:2}}>{m.l}</div>
                            </div>
                          ))}
                        </div>
                        {/* Barra de progresso */}
                        <div style={{marginTop:12,height:6,background:'#1a2540',borderRadius:3,overflow:'hidden'}}>
                          <div style={{height:'100%',width:`${pct}%`,background:`linear-gradient(90deg,${pct>=80?'#10b981':pct>=60?'#f59e0b':'#ef4444'},transparent)`,borderRadius:3,transition:'width .5s'}} />
                        </div>
                      </div>
                    )
                  })}
        ...
        {aba === 'regimes' && (
          <div>
            <div style={{marginBottom:16,fontSize:12,color:'#64748b'}}>
              Configure as obrigações de cada regime. As alterações serão aplicadas ao gerar novas obrigações.
            </div>
            <div style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap'}}>
              {REGIMES.map(r => (
                <button key={r.id}
                  onClick={() => setRegimeEditando(r.id)}
                  className={`tab-btn${regimeEditando === r.id ? ' active' : ''}`}>
                  {r.label}
                </button>
              ))}
            </div>
            <RegimeEditor
              regime={regimeEditando}
              obgBase={OBRIGACOES_BASE[regimeEditando] || []}
              customObg={customObgRegimes[regimeEditando] || null}
              onSalvar={lista => salvarCustomObgRegime(regimeEditando, lista)}
            />
          </div>
        )}
            ...
            <div className="card" style={{marginBottom:16}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:16}}>
                <div>
                  ...
                  <select value={tribUF} onChange={e => setTribUF(e.target.value)} className="field-input">
                    {UFS.map(uf => <option key={uf} value={uf}>{uf}</option>)}
                  </select>
                </div>
                <div>
                  <label className="field-label">📋 Regime Tributário</label>
                  <select value={tribRegime} onChange={e => setTribRegime(e.target.value)} className="field-input">
                    {REGIMES.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="field-label">🏢 Tipo de Negócio</label>
                  <select value={tribNegocio} onChange={e => setTribNegocio(e.target.value)} className="field-input">
                    {TIPOS_NEGOCIO.map(n => <option key={n.id} value={n.id}>{n.label}</option>)}
                  </select>
                </div>
              </div>
              <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>
                {[{id:'ncm',label:'🏷️ NCM'},{id:'ean',label:'📦 Cód. Barras'},{id:'descricao',label:'🔤 Descrição'}].map(t => (
                  <button key={t.id} onClick={() => { setTribTipo(t.id); setTribBusca(''); setTribResult(null); setTribErro(''); setTribLista([]) }}
                    style={{padding:'7px 16px',borderRadius:8,border:'1px solid',cursor:'pointer',fontFamily:'DM Mono,monospace',fontSize:12,fontWeight:600,
                      ...
                      color:tribTipo===t.id?'#00d4ff':'#64748b'}}>{t.label}</button>
                ))}
              </div>
              <div style={{display:'flex',gap:8}}>
                <input value={tribBusca} onChange={e => setTribBusca(e.target.value)}
                  onKeyDown={e => e.key==='Enter' && consultarTributaria()}
                  placeholder={tribTipo==='ncm'?'Ex: 8471.30.19':tribTipo==='ean'?'Ex: 7891234567890':'Ex: computador notebook'}
                  className="field-input" style={{flex:1}} />
                <button className="btn-primary" onClick={() => consultarTributaria()} disabled={tribLoading}>
                  {tribLoading ? '⏳...' : '🔍 Consultar'}
                </button>
              </div>
              {tribErro && <div style={{color:'#ef4444',fontSize:12,marginTop:8}}>{tribErro}</div>}
            </div>
          </div>
        )}

        {/* ══ OUTRAS ABAS (mantidas) ══ */}
        {aba === 'mei' && <div className="empty">🏪 Gestão MEI — em breve</div>}
        {aba === 'simples' && <div className="empty">📈 Simples Nacional — em breve</div>}
        {aba === 'ecac' && <div className="empty">🔍 Monitor e-CAC — em breve</div>}
        {aba === 'notas' && <div className="empty">📥 Captura de Notas — em breve</div>}
      </div>
...
// ─── Componente: Editor de obrigações por regime ──────────────────────────────
function RegimeEditor({ regime, obgBase, customObg, onSalvar }) {
  const [lista, setLista] = useState(customObg || obgBase.map(o => ({...o})))
  const [salvando, setSalvando] = useState(false)

  useEffect(() => {
    setLista(customObg || obgBase.map(o => ({...o})))
  }, [regime, customObg])

  function adicionarObg() {
    setLista(prev => [...prev, {
      id: 'custom_' + Date.now(),
      nome: '', tipo: 'Declaração', periodicidade: 'Mensal',
      dia_ideal: null, dia_atencao: null, dia_vencimento: null,
      categoria: 'Federal', mes_ref: null,
    }])
  }

  function removerObg(id) {
    setLista(prev => prev.filter(o => o.id !== id))
  }

  function updateObg(id, campo, valor) {
  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
        <div style={{fontSize:13,color:'#e2e8f0',fontWeight:600}}>{lista.length} obrigação(ões) no regime</div>
        <div style={{display:'flex',gap:8}}>
          <button onClick={adicionarObg} className="btn-primary" style={{fontSize:12}}>+ Adicionar</button>
          <button onClick={handleSalvar} disabled={salvando} className="btn-primary" style={{background:'linear-gradient(135deg,#10b981,#059669)',fontSize:12}}>
            {salvando?'⏳...':'💾 Salvar Regime'}
          </button>
        </div>
      </div>

      {lista.map((o, idx) => (
        <div key={o.id} style={{background:'#111827',border:'1px solid #1e2d4a',borderRadius:10,padding:'14px',marginBottom:8}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 120px 120px',gap:8,marginBottom:8}}>
            <input value={o.nome} onChange={e => updateObg(o.id,'nome',e.target.value)}
              className="field-input" style={{fontSize:12}} placeholder="Nome da obrigação" />
            <select value={o.categoria} onChange={e => updateObg(o.id,'categoria',e.target.value)} className="field-input" style={{fontSize:12}}>
              {CATEGORIAS.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select value={o.periodicidade} onChange={e => updateObg(o.id,'periodicidade',e.target.value)} className="field-input" style={{fontSize:12}}>
              {PERIODICIDADES.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 60px',gap:8,alignItems:'end'}}>
            <div>
              <label style={{fontSize:10,color:'#10b981',display:'block',marginBottom:3}}>✅ Dia Ideal</label>
              <input type="number" min="1" max="31" value={o.dia_ideal||''} onChange={e => updateObg(o.id,'dia_ideal',e.target.value?Number(e.target.value):null)} className="field-input" style={{fontSize:12}} placeholder="Ex: 10" />
            </div>
            <div>
              <label style={{fontSize:10,color:'#f59e0b',display:'block',marginBottom:3}}>⚠️ Dia Atenção</label>
              <input type="number" min="1" max="31" value={o.dia_atencao||''} onChange={e => updateObg(o.id,'dia_atencao',e.target.value?Number(e.target.value):null)} className="field-input" style={{fontSize:12}} placeholder="Ex: 13" />
            </div>
            <div>
              <label style={{fontSize:10,color:'#ef4444',display:'block',marginBottom:3}}>🔴 Vencimento</label>
              <input type="number" min="1" max="31" value={o.dia_vencimento||''} onChange={e => updateObg(o.id,'dia_vencimento',e.target.value?Number(e.target.value):null)} className="field-input" style={{fontSize:12}} placeholder="Ex: 15" />
            </div>
            {o.periodicidade === 'Anual' && (
              <div>
                <label style={{fontSize:10,color:'#64748b',display:'block',marginBottom:3}}>Mês de Ref.</label>
                <select value={o.mes_ref||''} onChange={e => updateObg(o.id,'mes_ref',e.target.value?Number(e.target.value):null)} className="field-input" style={{fontSize:12}}>
                  <option value="">-</option>
                  {MESES.map((m,i) => <option key={i} value={i+1}>{m}</option>)}
                </select>
              </div>
            )}
            <button onClick={() => removerObg(o.id)} style={{...btnSmStyle,color:'#ef4444',borderColor:'rgba(239,68,68,.3)',padding:'8px',alignSelf:'end'}}>🗑</button>
          </div>
        </div>
      ))}
      {lista.length === 0 && (
        <div className="empty">Nenhuma obrigação neste regime. Clique em "+ Adicionar".</div>
      )}
    </div>
  )
}
