...
function TabEmpresa({ cfg, setCfg, empresaId }) {
  ...
  const [nfeCertSenha, setNfeCertSenha] = useState(cfg.senhaCertificado  || '')
  const [nfeTpAmb,     setNfeTpAmb]     = useState(cfg.nfeTpAmb || '2')
  const [nfeIE,        setNfeIE]        = useState(cfg.nfeIE     || '')
  const [nfeIM,        setNfeIM]        = useState(cfg.nfeIM     || '')
  const [nfeCRT,       setNfeCRT]       = useState(cfg.nfeCRT    || '1')
  const [nfeLogradouro,setNfeLogradouro]= useState(cfg.logradouro   || '')
  const [nfeNumero,    setNfeNumero]    = useState(cfg.numeroEnd    || '')
  const [nfeBairro,    setNfeBairro]    = useState(cfg.bairro       || '')
  const [nfeMunicipio, setNfeMunicipio] = useState(cfg.municipio    || '')
  const [nfeCodMun,    setNfeCodMun]    = useState(cfg.codigoMunicipio || '')
  const [nfeUF,        setNfeUF]        = useState(cfg.uf           || '')
  const [nfeCEP,       setNfeCEP]       = useState(cfg.cep          || '')
  const [nfeSerie,     setNfeSerie]     = useState(cfg.nfeSerie     || '1')
  const [ultimoNumNFe, setUltimoNumNFe] = useState(cfg.ultimoNumeroNFe  || 0)
  const [ultimoNumNFCe,setUltimoNumNFCe]= useState(cfg.ultimoNumeroNFCe || 0)
  ...
  useEffect(() => {
    ...
    setNfeCertB64(cfg.certificadoBase64 || '')
    setNfeCertSenha(cfg.senhaCertificado || '')
    setNfeTpAmb(cfg.nfeTpAmb || '2')
    setNfeIE(cfg.nfeIE || '')
    setNfeIM(cfg.nfeIM || '')
    setNfeCRT(cfg.nfeCRT || '1')
    setNfeLogradouro(cfg.logradouro || '')
    setNfeNumero(cfg.numeroEnd || '')
    setNfeBairro(cfg.bairro || '')
    setNfeMunicipio(cfg.municipio || '')
    setNfeCodMun(cfg.codigoMunicipio || '')
    setNfeUF(cfg.uf || '')
    setNfeCEP(cfg.cep || '')
    setNfeSerie(cfg.nfeSerie || '1')
    setUltimoNumNFe(cfg.ultimoNumeroNFe || 0)
    setUltimoNumNFCe(cfg.ultimoNumeroNFCe || 0)
  }, [cfg])
  ...
  async function salvar() {
    ...
    const novoCfg = {
      ...
      // NF-e
      certificadoBase64: nfeCertB64,
      senhaCertificado: nfeCertSenha,
      nfeTpAmb, nfeIE, nfeIM, nfeCRT,
      logradouro: nfeLogradouro, numeroEnd: nfeNumero, bairro: nfeBairro,
      municipio: nfeMunicipio, codigoMunicipio: nfeCodMun, uf: nfeUF, cep: nfeCEP,
      nfeSerie, ultimoNumeroNFe: Number(ultimoNumNFe), ultimoNumeroNFCe: Number(ultimoNumNFCe)
    }
  ...
  return (
    <div style={s.body}>
      ...
      <div style={s.sec}>
        <div style={s.secTitle}>📍 Endereço Fiscal (para NF-e)</div>
        <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 12 }}>Endereço completo que aparecerá nas notas fiscais eletrônicas.</p>
        <div style={s.row2}>
          <div style={s.field}><label style={s.label}>Logradouro</label><input style={s.input} value={nfeLogradouro} onChange={e => setNfeLogradouro(e.target.value)} placeholder="Rua, Av, Travessa..." /></div>
          <div style={s.field}><label style={s.label}>Número</label><input style={s.input} value={nfeNumero} onChange={e => setNfeNumero(e.target.value)} placeholder="123" /></div>
        </div>
        <div style={s.row2}>
          <div style={s.field}><label style={s.label}>Bairro</label><input style={s.input} value={nfeBairro} onChange={e => setNfeBairro(e.target.value)} placeholder="Centro" /></div>
          <div style={s.field}><label style={s.label}>Município</label><input style={s.input} value={nfeMunicipio} onChange={e => setNfeMunicipio(e.target.value)} placeholder="Belo Horizonte" /></div>
        </div>
        <div style={s.row2}>
          <div style={s.field}><label style={s.label}>Código IBGE do Município</label><input style={s.input} value={nfeCodMun} onChange={e => setNfeCodMun(e.target.value)} placeholder="3106200" /></div>
          <div style={s.field}><label style={s.label}>UF</label>
            <select style={s.input} value={nfeUF} onChange={e => setNfeUF(e.target.value)}>
        ...
        <div style={s.field}><label style={s.label}>CEP</label><input style={s.input} value={nfeCEP} onChange={e => setNfeCEP(e.target.value)} placeholder="00000-000" /></div>
      </div>

      <div style={s.sec}>
        <div style={s.secTitle}>🏛️ Dados Fiscais da Empresa</div>
        <div style={s.row2}>
          <div style={s.field}><label style={s.label}>Inscrição Estadual (IE)</label><input style={s.input} value={nfeIE} onChange={e => setNfeIE(e.target.value)} placeholder="Ex: 0623347780095" /></div>
          <div style={s.field}><label style={s.label}>Inscrição Municipal (IM)</label><input style={s.input} value={nfeIM} onChange={e => setNfeIM(e.target.value)} placeholder="Para NFS-e" /></div>
        </div>
        <div style={s.field}>
          <label style={s.label}>Regime Tributário (CRT)</label>
          <select style={s.input} value={nfeCRT} onChange={e => setNfeCRT(e.target.value)}>
      ...
      <div style={s.sec}>
        <div style={s.secTitle}>🔐 Certificado Digital A1 (NF-e / NFC-e)</div>
        <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 12, lineHeight: 1.6 }}>
          Faça upload do arquivo <strong style={{ color: 'var(--text)' }}>.pfx</strong> do certificado digital A1 da empresa. Ele será usado para assinar e emitir notas fiscais eletrônicas.
        </p>
        <div style={s.field}>
          <label style={s.label}>Arquivo do Certificado (.pfx)</label>
          <input type="file" accept=".pfx" onChange={async (e) => {
            const file = e.target.files[0]
            if (!file) return
            const reader = new FileReader()
            reader.onload = (ev) => setNfeCertB64(ev.target.result.split(',')[1])
            reader.readAsDataURL(file)
          }} style={{ ...s.input, padding: '6px' }} />
          {nfeCertB64
            ? <div style={{ fontSize: 11, color: 'var(--accent3)', marginTop: 6 }}>✅ Certificado carregado ({Math.round(nfeCertB64.length * 0.75 / 1024)} KB)</div>
            : <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 6 }}>⚠️ Nenhum certificado carregado</div>
          }
        </div>
        <div style={s.field}>
          <label style={s.label}>Senha do Certificado</label>
          <input type="password" style={s.input} value={nfeCertSenha} onChange={e => setNfeCertSenha(e.target.value)} placeholder="Senha do arquivo .pfx" autoComplete="new-password" />
        </div>
        <div style={s.field}>
          <label style={s.label}>Ambiente de Emissão</label>
          <select style={s.input} value={nfeTpAmb} onChange={e => setNfeTpAmb(e.target.value)}>
            <option value="2">🟡 Homologação (Testes — não tem validade fiscal)</option>
            <option value="1">🟢 Produção (Notas com validade fiscal real)</option>
          </select>
          {nfeTpAmb === '1' && (
            <div style={{ marginTop: 8, padding: '8px 12px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.3)', borderRadius: 8, fontSize: 12, color: '#ef4444' }}>
              ⚠️ <strong>Atenção:</strong> Em produção as notas emitidas têm validade fiscal real perante a SEFAZ.
            </div>
          )}
        </div>
      </div>
      ...
      <div style={s.sec}>
        <div style={s.secTitle}>🔢 Numeração das Notas Fiscais</div>
        <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 12 }}>
          Estes números são incrementados automaticamente a cada nota emitida. Altere apenas se precisar sincronizar com emissões anteriores.
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
          <div style={s.field}>
            <label style={s.label}>Série NF-e / NFC-e</label>
            <input type="number" min={1} max={999} style={s.input} value={nfeSerie} onChange={e => setNfeSerie(e.target.value)} />
          </div>
          <div style={s.field}>
            <label style={s.label}>Último nº NF-e (mod.55)</label>
            <input type="number" min={0} style={s.input} value={ultimoNumNFe} onChange={e => setUltimoNumNFe(e.target.value)} />
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>Próxima: {Number(ultimoNumNFe) + 1}</div>
          </div>
          <div style={s.field}>
            <label style={s.label}>Último nº NFC-e (mod.65)</label>
            <input type="number" min={0} style={s.input} value={ultimoNumNFCe} onChange={e => setUltimoNumNFCe(e.target.value)} />
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>Próxima: {Number(ultimoNumNFCe) + 1}</div>
          </div>
        </div>
      </div>
...
// ══════════════════════════════════════════════
// ABA ERP — PRODUTOS E SERVIÇOS FISCAIS
// ══════════════════════════════════════════════
const PRODUTO_VAZIO = {
  id: '', tipo: 'produto', codigo: '', codigoBarras: '',
  descricao: '', unidade: 'UN', ncm: '', cest: '', cfopEntrada: '1102', cfopSaida: '5102',
  // ICMS
  orig: '0', csosn: '400', modBC: '3', aliqICMS: '12', redBC: '0',
  // IPI
  cstIPI: '50', aliqIPI: '0',
  // PIS
  cstPIS: '01', aliqPIS: '0.65',
  // COFINS
  cstCOFINS: '01', aliqCOFINS: '3',
  // Outros
  aliqCSLL: '1', aliqIRPJ: '1.2',
  precoVenda: '', precoCompra: '', estoque: '0',
  observacoes: ''
}

const SERVICO_VAZIO = {
  id: '', tipo: 'servico', codigo: '', descricao: '',
  cnae: '', lcServico: '', aliqISS: '5',
  // PIS / COFINS / CSLL / IR (retenção)
  cstPIS: '07', aliqPIS: '0',
  cstCOFINS: '07', aliqCOFINS: '0',
  retCSLL: false, aliqCSLL: '1',
  retIR: false, aliqIR: '1.5',
  retINSS: false, aliqINSS: '11',
  precoUnitario: '', unidade: 'UN',
  observacoes: ''
}

function TabErp({ cfg, setCfg, empresaId }) {
  const [subAba, setSubAba] = useState('produtos')
  const [itens,  setItens]  = useState(cfg.erpProdutos || [])
  const [servicos, setServicos] = useState(cfg.erpServicos || [])
  const [form,   setForm]   = useState(null)
  const [saving, setSaving] = useState(false)
  const [busca,  setBusca]  = useState('')

  useEffect(() => {
    setItens(cfg.erpProdutos || [])
    setServicos(cfg.erpServicos || [])
  }, [cfg])

  async function salvarItem() {
    if (!form.descricao) { toast('Descrição obrigatória', 'err'); return }
    setSaving(true)
    const isServico = subAba === 'servicos'
    const lista = isServico ? [...servicos] : [...itens]
    const item = { ...form, id: form.id || String(Date.now()) }
    const idx = lista.findIndex(x => x.id === item.id)
    if (idx >= 0) lista[idx] = item
    else lista.push(item)
    const novoCfg = isServico
      ? { ...cfg, erpServicos: lista }
      : { ...cfg, erpProdutos: lista }
    const { error } = await salvarStorage(empresaId, novoCfg)
    setSaving(false)
  ...
  async function excluirItem(id) {
    ...
    const isServico = subAba === 'servicos'
    const lista = (isServico ? servicos : itens).filter(x => x.id !== id)
    const novoCfg = isServico
      ? { ...cfg, erpServicos: lista }
      : { ...cfg, erpProdutos: lista }
    await salvarStorage(empresaId, novoCfg)
  ...
  const listaAtiva = subAba === 'servicos' ? servicos : itens
  const listaFiltrada = listaAtiva.filter(x =>
    !busca || x.descricao?.toLowerCase().includes(busca.toLowerCase()) || x.codigo?.includes(busca)
  )
    ...
    <div style={s.field}>
      ...
      {opts.select
        ? <select style={s.input} value={form[key] || ''} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}>
            {opts.select.map(([v, l]) => <option key={v} value={v}>{v} — {l}</option>)}
          </select>
        : opts.check
        ? <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', paddingTop: 8 }}>
            <input type="checkbox" checked={!!form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.checked }))} />
            <span style={{ fontSize: 12, color: 'var(--muted)' }}>{opts.checkLabel}</span>
          </label>
        : <input type={opts.type || 'text'} style={s.input} value={form[key] ?? ''} placeholder={opts.placeholder || ''}
  ...
  return (
    <div style={s.body}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {[['produtos','📦 Produtos'],['servicos','🛠️ Serviços']].map(([id,label]) => (
          <button key={id} onClick={() => { setSubAba(id); setForm(null) }}
            style={{ padding: '9px 18px', borderRadius: 9, border: '1px solid var(--border)',
              background: subAba === id ? 'rgba(0,212,255,.12)' : 'var(--surface2)',
              color: subAba === id ? 'var(--accent)' : 'var(--muted)',
              fontFamily: 'DM Mono, monospace', fontSize: 12, cursor: 'pointer', fontWeight: subAba === id ? 700 : 400 }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {!form && (
        <div style={s.sec}>
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center' }}>
            <input style={{ ...s.input, flex: 1 }} placeholder="🔍 Buscar por descrição ou código..." value={busca} onChange={e => setBusca(e.target.value)} />
            <button onClick={() => setForm(subAba === 'servicos' ? { ...SERVICO_VAZIO } : { ...PRODUTO_VAZIO })}
              style={{ padding: '9px 16px', borderRadius: 9, background: 'rgba(0,212,255,.1)', border: '1px solid rgba(0,212,255,.3)', color: 'var(--accent)', fontFamily: 'DM Mono, monospace', fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              + {subAba === 'servicos' ? 'Novo Serviço' : 'Novo Produto'}
            </button>
          </div>

          <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 10 }}>
            {listaFiltrada.length} {subAba === 'servicos' ? 'serviço(s)' : 'produto(s)'} cadastrado(s)
          </div>

          {listaFiltrada.length === 0 ? (
            <div style={{ padding: '32px', textAlign: 'center', color: 'var(--muted)', fontSize: 13, border: '1px dashed var(--border)', borderRadius: 10 }}>
              Nenhum {subAba === 'servicos' ? 'serviço' : 'produto'} cadastrado. Clique em "+ Novo" para começar.
            </div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr>
                    {['Código','Descrição', subAba === 'servicos' ? 'LC/CNAE' : 'NCM', subAba === 'servicos' ? 'ISS%' : 'CFOP Saída', 'Preço','Ações'].map(h => (
                      <th key={h} style={{ background: 'var(--surface2)', padding: '8px 10px', textAlign: 'left', color: 'var(--muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: .5, borderBottom: '1px solid var(--border)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {listaFiltrada.map(item => (
                    <tr key={item.id} style={{ borderBottom: '1px solid rgba(30,45,74,.4)' }}>
                      <td style={{ padding: '8px 10px', color: 'var(--accent)', fontWeight: 600 }}>{item.codigo || '—'}</td>
                      <td style={{ padding: '8px 10px', color: 'var(--text)', maxWidth: 200 }}>{item.descricao}</td>
                      <td style={{ padding: '8px 10px', color: 'var(--muted)' }}>{subAba === 'servicos' ? (item.lcServico || item.cnae || '—') : (item.ncm || '—')}</td>
                      <td style={{ padding: '8px 10px', color: 'var(--muted)' }}>{subAba === 'servicos' ? (item.aliqISS + '%') : (item.cfopSaida || '—')}</td>
                      <td style={{ padding: '8px 10px', color: 'var(--accent3)' }}>
                        {item.precoVenda || item.precoUnitario ? `R$ ${Number(item.precoVenda || item.precoUnitario).toFixed(2)}` : '—'}
                      </td>
                      <td style={{ padding: '8px 10px' }}>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button onClick={() => setForm({ ...item })} style={{ padding: '4px 8px', borderRadius: 6, background: 'rgba(0,212,255,.1)', border: '1px solid rgba(0,212,255,.2)', color: 'var(--accent)', cursor: 'pointer', fontSize: 11 }}>✏️</button>
                          <button onClick={() => excluirItem(item.id)} style={{ padding: '4px 8px', borderRadius: 6, background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', color: 'var(--danger)', cursor: 'pointer', fontSize: 11 }}>🗑</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {/* FORMULÁRIO PRODUTO */}
      {form && subAba === 'produtos' && (
        <div style={s.sec}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={s.secTitle}>{form.id ? '✏️ Editar Produto' : '➕ Novo Produto'}</div>
            <button onClick={() => setForm(null)} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 20 }}>✕</button>
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10, paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>📦 Dados Gerais</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {inp('Código Interno', 'codigo', { placeholder: 'Ex: PROD001' })}
            {inp('Código de Barras / GTIN', 'codigoBarras', { placeholder: 'EAN-13 ou SEM GTIN' })}
            {inp('Unidade Comercial', 'unidade', { placeholder: 'UN, KG, CX...' })}
          </div>
          <div style={s.field}><label style={s.label}>Descrição do Produto</label>
            <input style={s.input} value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} placeholder="Descrição completa que sairá na NF-e" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {inp('NCM (8 dígitos)', 'ncm', { placeholder: '00000000' })}
            {inp('CEST (7 dígitos, se houver)', 'cest', { placeholder: '0000000' })}
            {inp('Origem da Mercadoria', 'orig', { select: [['0','Nacional'],['1','Estrangeira (importação direta)'],['2','Estrangeira (mercado interno)'],['3','Nacional c/ conteúdo importação >40%'],['4','Nacional, processos prod. básicos'],['5','Nacional, conteúdo import. >70%'],['6','Estrangeira (importação direta) sem similar'],['7','Estrangeira (merc. interno) sem similar'],['8','Nacional, conteúdo import. >40% e ≤70%']] })}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {inp('CFOP de Entrada', 'cfopEntrada', { placeholder: 'Ex: 1102' })}
            {inp('CFOP de Saída', 'cfopSaida', { placeholder: 'Ex: 5102' })}
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, margin: '18px 0 10px', paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>🏛️ ICMS (Simples Nacional)</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {inp('CSOSN', 'csosn', { select: [
              ['101','Tributada com permissão de crédito'],
              ['102','Tributada sem permissão de crédito'],
              ['103','Isenção do ICMS p/ faixa de receita bruta'],
              ['201','Com ST e permissão de crédito'],
              ['202','Com ST sem permissão de crédito'],
              ['203','Com ST — isenção p/ faixa de rec. bruta'],
              ['300','Imune'],
              ['400','Não tributada pelo Simples Nacional'],
              ['500','ICMS cobrado anteriormente por ST'],
              ['900','Outros'],
            ]})}
            {inp('Modalidade Base Cálculo', 'modBC', { select: [['0','Margem Valor Agregado'],['1','Pauta'],['2','Preço Tabelado Máx.'],['3','Valor da Operação']] })}
            {inp('Alíquota ICMS (%)', 'aliqICMS', { type: 'number', placeholder: '12' })}
          </div>
          {inp('Redução Base Cálculo ICMS (%)', 'redBC', { type: 'number', placeholder: '0' })}

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, margin: '18px 0 10px', paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>🏭 IPI</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {inp('CST IPI', 'cstIPI', { select: [['00','Entrada c/ recuperação de crédito'],['49','Outras entradas'],['50','Saída tributada'],['99','Outras saídas']] })}
            {inp('Alíquota IPI (%)', 'aliqIPI', { type: 'number', placeholder: '0' })}
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, margin: '18px 0 10px', paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>💼 PIS / COFINS</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12 }}>
            {inp('CST PIS', 'cstPIS', { select: [['01','Alíquota normal (cumulativo)'],['02','Alíquota diferenciada'],['07','Isento'],['49','Outras entradas'],['50','Alíquota básica'],['70','Alíquota zero'],['99','Outras saídas']] })}
            {inp('Alíquota PIS (%)', 'aliqPIS', { type: 'number', placeholder: '0.65' })}
            {inp('CST COFINS', 'cstCOFINS', { select: [['01','Alíquota normal (cumulativo)'],['02','Alíquota diferenciada'],['07','Isento'],['49','Outras entradas'],['50','Alíquota básica'],['70','Alíquota zero'],['99','Outras saídas']] })}
            {inp('Alíquota COFINS (%)', 'aliqCOFINS', { type: 'number', placeholder: '3' })}
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, margin: '18px 0 10px', paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>💲 Preços e Estoque</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {inp('Preço de Venda (R$)', 'precoVenda', { type: 'number', placeholder: '0.00' })}
            {inp('Preço de Compra (R$)', 'precoCompra', { type: 'number', placeholder: '0.00' })}
            {inp('Estoque Inicial', 'estoque', { type: 'number', placeholder: '0' })}
          </div>
          {inp('Observações / Informações Adicionais NF-e', 'observacoes', { placeholder: 'Texto que aparecerá no campo de inf. adicionais da NF-e' })}

          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
            <button style={s.saveBtn} onClick={salvarItem} disabled={saving}>{saving ? '⏳ Salvando...' : '✅ Salvar Produto'}</button>
            <button onClick={() => setForm(null)} style={{ padding: '11px 18px', borderRadius: 10, background: 'rgba(100,116,139,.12)', border: '1px solid rgba(100,116,139,.3)', color: 'var(--muted)', fontFamily: 'DM Mono, monospace', fontSize: 13, cursor: 'pointer' }}>Cancelar</button>
          </div>
        </div>
      )}
      ...
      {/* FORMULÁRIO SERVIÇO */}
      {form && subAba === 'servicos' && (
        <div style={s.sec}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={s.secTitle}>{form.id ? '✏️ Editar Serviço' : '➕ Novo Serviço'}</div>
            <button onClick={() => setForm(null)} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 20 }}>✕</button>
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10, paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>🛠️ Dados Gerais</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {inp('Código Interno', 'codigo', { placeholder: 'Ex: SRV001' })}
            {inp('Unidade', 'unidade', { placeholder: 'UN, HR, MÊS...' })}
          </div>
          <div style={s.field}><label style={s.label}>Descrição do Serviço</label>
            <input style={s.input} value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} placeholder="Descrição completa que sairá na NFS-e" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {inp('LC 116/03 — Item', 'lcServico', { placeholder: 'Ex: 1.07' })}
            {inp('CNAE', 'cnae', { placeholder: 'Ex: 6201-5/01' })}
            {inp('Alíquota ISS (%)', 'aliqISS', { type: 'number', placeholder: '5' })}
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, margin: '18px 0 10px', paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>💼 PIS / COFINS (retenção)</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12 }}>
            {inp('CST PIS', 'cstPIS', { select: [['07','Isento'],['01','Alíquota normal'],['70','Alíquota zero'],['99','Outros']] })}
            {inp('Alíquota PIS (%)', 'aliqPIS', { type: 'number', placeholder: '0' })}
            {inp('CST COFINS', 'cstCOFINS', { select: [['07','Isento'],['01','Alíquota normal'],['70','Alíquota zero'],['99','Outros']] })}
            {inp('Alíquota COFINS (%)', 'aliqCOFINS', { type: 'number', placeholder: '0' })}
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, margin: '18px 0 10px', paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>🔒 Retenções na Fonte</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              {inp('Reter CSLL?', 'retCSLL', { check: true, checkLabel: 'Sim, reter CSLL' })}
              {form.retCSLL && inp('Alíquota CSLL (%)', 'aliqCSLL', { type: 'number', placeholder: '1' })}
            </div>
            <div>
              {inp('Reter IRPJ?', 'retIR', { check: true, checkLabel: 'Sim, reter IRPJ' })}
              {form.retIR && inp('Alíquota IRPJ (%)', 'aliqIR', { type: 'number', placeholder: '1.5' })}
            </div>
            <div>
              {inp('Reter INSS?', 'retINSS', { check: true, checkLabel: 'Sim, reter INSS' })}
              {form.retINSS && inp('Alíquota INSS (%)', 'aliqINSS', { type: 'number', placeholder: '11' })}
            </div>
          </div>

          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: 1, margin: '18px 0 10px', paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>💲 Preço</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {inp('Preço Unitário (R$)', 'precoUnitario', { type: 'number', placeholder: '0.00' })}
          </div>
          {inp('Observações / Informações NFS-e', 'observacoes', { placeholder: 'Informações adicionais para a nota de serviço' })}

          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
            <button style={s.saveBtn} onClick={salvarItem} disabled={saving}>{saving ? '⏳ Salvando...' : '✅ Salvar Serviço'}</button>
            <button onClick={() => setForm(null)} style={{ padding: '11px 18px', borderRadius: 10, background: 'rgba(100,116,139,.12)', border: '1px solid rgba(100,116,139,.3)', color: 'var(--muted)', fontFamily: 'DM Mono, monospace', fontSize: 13, cursor: 'pointer' }}>Cancelar</button>
          </div>
        </div>
      )}
