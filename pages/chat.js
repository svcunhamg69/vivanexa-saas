...
// ============================================================
// CORREÇÕES APLICADAS:
// 1. Timer reseta ao clicar em "Encerrar e iniciar nova consulta"
// 2. Proposta: "em até 10× sem juros" (não "10×")
// 3. Modo voucher: chat pede o código antes de calcular desconto
// 4. Dados do cliente preenchem corretamente no modal (empresa,
//    razão, email, telefone, cidade, uf via clientData + contactData)
// 5. Menus de navegação visíveis e funcionais no header
// ============================================================
...
function calcDisc(mods,plan,ifPlan,cnpjs,notas,cfg,voucherOverride){
  // voucherOverride: {discAdPct, discMenPct} se vier de voucher
  ...
  for(const mod of mods){
    if(mod==='IF'){const p=ifPlan||'basic',[aB,mB]=getPrice('IF',p,cfg),ad=aB*2,men=mB*1.2,adD=aB*(1-adPct/100),menD=mB*(1-menPct/100);res.push({name:pn('IF',cfg),ad,men,adD,menD,isPrepaid:true,plan:p,isIF:true});tAd+=ad;tMen+=men;tAdD+=adD;tMenD+=menD;continue}
    ...
    if(mod==='EP'){const ep=plan==='topplus'?'top':plan,[,mB]=getPrice('EP',ep,cfg),men=mB*1.2,menD=mB*(1-menPct/100);res.push({name:pn('EP',cfg),ad:0,men,adD:0,menD,isEP:true,plan:ep});tMen+=men;tMenD+=menD;continue}
    const[aB,mB]=getPrice(mod,plan,cfg);let ad=aB>0?Math.max(aB*2,1000):0,men=mB*1.2
    if(mod==='XML')men=Math.max(men,175);if(mod==='Gestão Fiscal')men=Math.max(men,200)
...
function buildProposal(S,cfg,user){
  ...
  const rows=(results||[]).map(r=>{
    ...
    return`<tr><td style="padding:10px 14px"><div style="font-weight:600;color:#0f172a">${r.name}</div>${r.plan?`<div style="font-size:11px;color:#64748b">Plano ${getPlanLabel(r.plan,cfg.plans)}</div>`:''}</td><td style="padding:10px 14px;text-align:center">${adS}</td><td style="padding:10px 14px;text-align:center">${fmt(isC?r.men:r.menD)}</td></tr>`
  }).join('')
  ...
  <div style="background:linear-gradient(135deg,#0f172a,#1e3a5f);padding:32px 44px">
    ${cfg.logob64?`<img src="${cfg.logob64}" style="height:50px;object-fit:contain;margin-bottom:10px;display:block">`:
      `<div style="font-size:24px;font-weight:900;color:#00d4ff;letter-spacing:2px;margin-bottom:10px">${cfg.company||'Vivanexa'}</div>`}
  ...
  <div style="padding:32px 44px">
    <div style="margin-bottom:26px">${sec('Dados do Cliente')}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        ${field('Empresa',co.empresa||cd.fantasia||cd.nome||'')}
        ${field('Razão Social',co.razao||cd.nome||'')}
        ${field('CPF / CNPJ',fmtDoc(S.doc||''))}
        ${field('Nome do Contato',co.contato||'')}
        ${field('E-mail',co.email||cd.email||'')}
        ${field('Telefone',co.telefone||cd.telefone||'')}
        ${field('Cidade',co.cidade||cd.municipio||'')}
        ${field('Estado',co.uf||cd.uf||'')}
      </div>
    </div>
    <div style="margin-bottom:26px">${sec('Módulos Contratados')}
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <thead><tr style="background:#f8fafc">
          <th style="padding:10px 14px;text-align:left;font-size:10px;text-transform:uppercase;color:#64748b;border-bottom:2px solid #e2e8f0">Módulo</th>
          <th style="padding:10px 14px;text-align:center;font-size:10px;text-transform:uppercase;color:#64748b;border-bottom:2px solid #e2e8f0">Adesão</th>
          <th style="padding:10px 14px;text-align:center;font-size:10px;text-transform:uppercase;color:#64748b;border-bottom:2px solid #e2e8f0">Mensalidade</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
    ...
    <div style="margin-bottom:26px">${sec('Condições de Pagamento — Adesão')}
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px">
        <div style="border:2px solid #00d4ff;border-radius:10px;padding:14px;text-align:center">
          <div style="font-size:22px;margin-bottom:6px">💳</div>
          <div style="font-size:11px;font-weight:700;color:#0f172a;text-transform:uppercase;margin-bottom:6px">Cartão de Crédito</div>
          <div style="font-size:11px;color:#64748b">Parcelado em</div>
          <div style="font-size:12px;color:#10b981;font-weight:600;margin-top:4px">em até 10× sem juros</div>
          <div style="font-size:11px;color:#64748b;margin-top:4px">${fmt(tAd/10)} / mês</div>
        </div>
        <div style="border:1px solid #e2e8f0;border-radius:10px;padding:14px;text-align:center">
          <div style="font-size:22px;margin-bottom:6px">🏦</div>
          <div style="font-size:11px;font-weight:700;color:#0f172a;text-transform:uppercase;margin-bottom:6px">Boleto / PIX</div>
          <div style="font-size:11px;color:#64748b">À vista</div>
          <div style="font-size:12px;color:#10b981;font-weight:600;margin-top:4px">${fmt(tAd)}</div>
        </div>
        <div style="border:1px solid #e2e8f0;border-radius:10px;padding:14px;text-align:center">
          <div style="font-size:22px;margin-bottom:6px">📄</div>
          <div style="font-size:11px;font-weight:700;color:#0f172a;text-transform:uppercase;margin-bottom:6px">Boleto Parcelado</div>
          <div style="font-size:11px;color:#64748b">3× sem juros</div>
          <div style="font-size:12px;color:#10b981;font-weight:600;margin-top:4px">${fmt(tAd*0.5)} entrada</div>
          <div style="font-size:11px;color:#64748b;margin-top:4px">+ ${fmt(tAd*0.25)} em 30/60 dias</div>
        </div>
      </div>
    </div>
...
function buildContract(S,cfg,user,tAd,tMen,dateAd,dateMen,payMethod){
  ...
  const rows=(results||[]).map(r=>{
    ...
    return`<tr><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0">${r.name}${r.plan?`<br><span style="font-size:11px;color:#64748b">Plano ${getPlanLabel(r.plan,cfg.plans)}</span>`:''}</td><td style="padding:8px 12px;text-align:center;border-bottom:1px solid #e2e8f0">${adS}</td><td style="padding:8px 12px;text-align:center;border-bottom:1px solid #e2e8f0">${fmt(isC?r.men:(r.menD||r.men||0))}</td></tr>`
  }).join('')
  const sec=t=>`<div style="font-size:13px;font-weight:700;color:#0f172a;margin:20px 0 10px;padding-bottom:6px;border-bottom:2px solid #e2e8f0">${t}</div>`
  const field=(l,v)=>`<div style="margin-bottom:8px"><span style="font-size:10px;color:#64748b;text-transform:uppercase;display:block;margin-bottom:2px">${l}</span><span style="font-size:13px;color:#1e293b">${v||'—'}</span></div>`
  return`<div style="background:#fff;font-family:Inter,sans-serif;color:#1e293b;max-width:820px;margin:0 auto;font-size:13px;line-height:1.6">
  <div style="background:linear-gradient(135deg,#0f172a,#1e3a5f);padding:28px 44px;display:flex;justify-content:space-between;align-items:center">
    <div>
      ${cfg.logob64?`<img src="${cfg.logob64}" style="height:44px;object-fit:contain;display:block;margin-bottom:6px">`:
        `<div style="font-size:20px;font-weight:900;color:#00d4ff;letter-spacing:2px;margin-bottom:6px">${cfg.company||'Vivanexa'}</div>`}
      <div style="font-size:10px;color:#64748b;letter-spacing:2px;text-transform:uppercase">Contrato de Prestação de Serviços</div>
    </div>
    <div style="text-align:right">
      <div style="font-size:11px;color:#64748b">Data</div>
      <div style="font-size:13px;color:#e2e8f0;font-weight:500">${today}</div>
    </div>
  </div>
  <div style="padding:28px 44px">
    ${sec('1. CONTRATADA')}
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${field('Empresa',cfg.company||'Vivanexa')}
      ${field('Site','www.vivanexa.com.br')}
      ${field('E-mail','contato@vivanexa.com.br')}
      ${field('Telefone','(69) 98405-9125')}
    </div>
    ${sec('2. CONTRATANTE')}
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      ${field('Empresa',co.empresa||cd.fantasia||cd.nome||'')}
      ${field('Razão Social',co.razao||cd.nome||'')}
      ${field('CNPJ/CPF',fmtDoc(S.doc||''))}
      ${field('Regime Tributário',co.regime||'')}
      ${field('E-mail',co.email||cd.email||'')}
      ${field('Telefone',co.telefone||cd.telefone||'')}
      ${field('Cidade / Estado',(co.cidade||cd.municipio||'')+(co.uf||cd.uf?` / ${co.uf||cd.uf}`:''))}
    </div>
    ${sec('3. RESPONSÁVEL PELA IMPLEMENTAÇÃO')}
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
      ${field('Nome',co.rimpNome||'')}
      ${field('E-mail',co.rimpEmail||'')}
      ${field('Telefone',co.rimpTel||'')}
    </div>
    ${sec('4. RESPONSÁVEL FINANCEIRO')}
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
      ${field('Nome',co.rfinNome||'')}
      ${field('E-mail',co.rfinEmail||'')}
      ${field('Telefone',co.rfinTel||'')}
    </div>
    ${sec('5. PLANO E VALORES')}
    <table style="width:100%;border-collapse:collapse;margin-bottom:14px">
      <thead><tr style="background:#f8fafc">
        <th style="padding:8px 12px;text-align:left;font-size:10px;color:#64748b;text-transform:uppercase;border-bottom:2px solid #e2e8f0">Módulo</th>
        <th style="padding:8px 12px;text-align:center;font-size:10px;color:#64748b;text-transform:uppercase;border-bottom:2px solid #e2e8f0">Adesão</th>
        <th style="padding:8px 12px;text-align:center;font-size:10px;color:#64748b;text-transform:uppercase;border-bottom:2px solid #e2e8f0">Mensalidade</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
    <div style="background:#f8fafc;border-radius:8px;padding:12px 16px;margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#64748b">Total Adesão</span><span style="font-weight:700">${fmt(tAd)}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#64748b">Total Mensalidade</span><span style="font-weight:700;color:#00d4ff">${fmt(tMen)}</span></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:16px">
      ${field('Forma de Pagamento — Adesão',payLabel)}
      ${tAd>0?field('Vencimento da Adesão',dateAd||'—'):''}
      ${field('Vencimento 1ª Mensalidade',dateMen||'—')}
    </div>
    ${sec('6. TERMOS E CONDIÇÕES')}
    <p style="color:#475569;font-size:12px;margin-bottom:8px">O presente contrato rege a prestação de serviços de software (SaaS) pela CONTRATADA à CONTRATANTE, nos módulos e valores acima descritos. O contrato tem vigência mínima de 12 (doze) meses, com renovação automática salvo aviso com 30 dias de antecedência.</p>
    <p style="color:#475569;font-size:12px;margin-bottom:8px">O cancelamento antes do término do período mínimo implica multa rescisória de 30% sobre o valor restante do contrato. Os valores de mensalidade poderão ser reajustados anualmente pelo IGPM ou índice equivalente.</p>
    <p style="color:#475569;font-size:12px;margin-bottom:20px">A CONTRATANTE concorda com os termos de uso e política de privacidade disponíveis em www.vivanexa.com.br/termos.</p>
    ${sec('7. ASSINATURAS')}
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:20px">
      <div style="text-align:center">
        <div style="border-top:1px solid #0f172a;padding-top:8px;margin-top:60px">
          <div style="font-weight:600">${cfg.company||'Vivanexa'}</div>
          <div style="font-size:12px;color:#64748b">CONTRATADA</div>
        </div>
      </div>
      <div style="text-align:center">
        <div style="border-top:1px solid #0f172a;padding-top:8px;margin-top:60px">
          <div style="font-weight:600">${co.empresa||cd.fantasia||cd.nome||'Cliente'}</div>
          <div style="font-size:12px;color:#64748b">CONTRATANTE</div>
        </div>
      </div>
    </div>
    <div style="text-align:center;margin-top:24px;font-size:11px;color:#94a3b8">${today}</div>
  </div>
</div>`
}
...
export default function Chat(){
  ...
  // CORREÇÃO 1: timerVal e timerDeadline controláveis para reset
  ...
  // Estado da conversa (ref para não causar re-renders)
  const S = useRef({
  ...
  async function loadCfg(id){
    try{
      const{data:row}=await supabase.from('vx_storage').select('value').eq('key',`cfg:${id}`).single()
      if(row?.value){
        try{
          const saved=JSON.parse(row.value)
          const nc={...DEFAULT_CFG,...saved}
          if(!nc.plans?.length) nc.plans=DEFAULT_CFG.plans
          if(!nc.prices || !Object.keys(nc.prices).length) nc.prices=DEFAULT_CFG.prices
          setCfg(nc)
        }catch{}
      }
  ...
  // CORREÇÃO 1: reset também limpa o timer
  const resetS = ()=>{
    ...
    setTimerDeadline(null)  // ← CORREÇÃO: zera o timer
    setTimerVal('')
  }
  ...
  async function processInput(t){
    ...
    if(['valeu','obrigado','obrigada','tchau'].some(w=>lo.includes(w))&&S.stage!=='await_doc'){
      S.stage='closed';return{h:false,c:'Perfeito! Boas vendas! 🚀'}
    }

    // CORREÇÃO 3: modo voucher — aguarda código antes de calcular desconto
    if(S.awaitingVoucher){
      const code=t.trim().toUpperCase()
      const voucher=(cfg.vouchers||[]).find(v=>v.codigo===code&&v.ativo!==false)
      if(!voucher){return{h:false,c:`❌ Voucher **${code}** não encontrado ou inativo.\n\nTente outro código ou digite "sem voucher" para pular:`}}
      if(lo.includes('sem voucher')||lo.includes('pular')){
        S.awaitingVoucher=false
        S.quoteData=calcFull(S.modules,S.plan,S.ifPlan,S.cnpjs,S.notas,cfg)
        S.stage='full_quoted'
        return{h:true,c:rFull(S.quoteData)}
      }
      S.appliedVoucher=voucher
      S.awaitingVoucher=false
      S.stage='discounted'
      ...
      addUser(t)
      addBot(`✅ Voucher **${voucher.codigo}** aplicado!\n\nDesconto: Adesão ${voucher.pctAdesao}% · Mensalidade ${voucher.pctMensalidade}%`)
      setTimeout(()=>addBot(rDisc(discData,getNextDates(),cn),true),300)
      return null
    }

    if(S.stage==='await_doc'){
      const doc=clean(t)
      if(!isCNPJ(doc)&&!isCPF(doc)) return{h:false,c:'Por favor, informe o CPF ou CNPJ (somente números).'}
      S.doc=doc
      if(isCNPJ(doc)){
        setThinking(true);const cd=await fetchCNPJ(doc);setThinking(false)
    ...
    if(S.stage==='await_users'){
      ...
      return{h:false,c:`👥 ${u} usuário${u>1?'s':''} registrado${u>1?'s':''}!\n\nQuais módulos?\n(${pn('Gestão Fiscal',cfg)} · ${pn('BIA',cfg)} · ${pn('CND',cfg)} · ${pn('XML',cfg)} · ${pn('IF',cfg)} · ${pn('EP',cfg)} · Tributos)`}
    }
    if(S.stage==='await_if_plan'){
      const p=parseIFPlan(t,cfg.plans);if(!p)return{h:false,c:`Informe o plano ${pn('IF',cfg)}:\n(${cfg.plans.map(x=>x.name).join(', ')})`}
      S.ifPlan=p;S.stage='await_modules';return checkCalc()
    }
    if(S.stage==='await_notas'){
      const n=parseInt(t);if(!n||n<1)return{h:false,c:'Quantas notas fiscais por mês?'}
      S.notas=n;S.stage='await_modules';return checkCalc()
    }
  ...
  function checkCalc(){
    ...
    // CORREÇÃO 3: se modo voucher, pede código antes de mostrar preço
    if(cfg.discMode==='voucher'&&!S.appliedVoucher&&!S.awaitingVoucher){
      S.awaitingVoucher=true
      return{h:false,c:`🎫 Esta empresa usa desconto via Voucher.\n\nDigite o **código do voucher** para aplicar o desconto:\n(ou "sem voucher" para ver o preço cheio)`}
    }
  ...
  // ── Handlers globais (chamados via onclick no HTML injetado) ─
  useEffect(()=>{
    const dates=getNextDates()
    ...
    window.vx_disc=(yes)=>{
      ...
      if(yes){
        S.stage='discounted'
        S.quoteData=calcDisc(S.modules,S.plan,S.ifPlan,S.cnpjs,S.notas,cfg,null)
        addUser('✅ Sim, quero ver!')
        addBot(rDisc(S.quoteData,dates,cn),true)
      }else{
        S.stage='closed';addUser('Não, obrigado');addBot('Sem problemas! 😊')
        setTimeout(()=>addBot(`<button class="reset-btn" onclick="window.vx_reset()">🔄 Iniciar nova consulta</button>`,true),400)
      }
      setChips([])
    }
    window.vx_close=(yes)=>{
      if(yes){
        const d=calcClose(S.modules,S.plan,S.ifPlan,S.cnpjs,S.notas,cfg)
        S.closingData=d;S.closingToday=true;S.stage='closing'
        addUser('✅ Sim, fechar hoje!')
        const dl=new Date();dl.setHours(18,0,0,0);setTimerDeadline(dl)
        addBot(rClose(d),true)
      }else{
        S.stage='discounted';addUser('Não por agora')
        addBot('Entendido! Valores com desconto padrão permanecem válidos.')
        setTimeout(()=>addBot(`<button class="reset-btn" onclick="window.vx_reset()">🔄 Iniciar nova consulta</button>`,true),400)
      }
      setChips([])
    }
    // CORREÇÃO 1: reset limpa timer (já feito na função resetS)
    window.vx_reset=()=>{
      resetS()
      setChips([])
      addBot('🔄 Nova consulta!\n\nInforme o CPF ou CNPJ do próximo cliente:')
    }
    // CORREÇÃO 4: preenche dados do cliente corretamente no modal
    window.vx_prop=()=>{
      const cd=S.clientData||{},co=S.contactData||{}
      setCf(f=>({
        ...f,
  ...
  function saveClient(){
    ...
    if(clientMode==='proposta'){
      openPrint(buildProposal(S,cfg,userProfile),'Proposta Comercial')
    }else{
      const isC=S.closingToday===true
      const tAd =isC?S.closingData?.tAd :(S.quoteData?.tAdD||0)
      const tMen=isC?S.closingData?.tMen:(S.quoteData?.tMenD||0)
  ...
  return(<>
    ...
    {/* CORREÇÃO 5: HEADER COM MENUS VISÍVEIS */}
    <header>
      ...
      <nav style={{display:'flex',gap:6,alignItems:'center',marginLeft:'auto'}}>
        <button onClick={()=>router.push('/dashboard')}
          style={{background:'var(--surface2)',border:'1px solid var(--border)',cursor:'pointer',color:'var(--muted)',fontSize:11,padding:'5px 11px',borderRadius:8,fontFamily:'DM Mono,monospace',letterSpacing:.3,transition:'color .2s'}}
        ...
        <span className="header-user">👤 <span>{userProfile.nome}</span></span>
    ...
    <div className="chat-wrap">
      <div id="messages" ref={msgRef}>
        ...
        {/* CORREÇÃO 1: timer só aparece quando há deadline ativo */}
        ...
        <div className="modal-box">
          ...
          <div className="modal-body">
            ...
            <div className="modal-grid2">
              {[['Empresa (Nome Fantasia)','empresa'],['Razão Social','razao'],['Nome do Contato','contato'],['E-mail','email'],['Telefone','telefone'],['Cidade','cidade'],['Estado (UF)','uf'],['CPF do Contato','cpfContato'],['Regime Tributário','regime']].map(([l,k])=>(
        ...
        <div className="modal-box" style={{maxWidth:680}}>
          ...
          <div className="modal-body">
            {wizStep===1&&<>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                {Array.from({length:10},(_,i)=>i+1).map(n=>(
                  <div key={n} className={`pay-opt sm${wizPay===`cartao${n}x`?' sel':''}`} onClick={()=>setWizPay(`cartao${n}x`)}>
                    ...
                    <div style={{flex:1}}><div className="po-t">{n}× sem juros</div><div className="po-s">no cartão</div></div>
              ...
              {[
                {n:2,desc:`Entrada ${fmt(wizTAd*0.5)} + ${fmt(wizTAd*0.5)} em 30 dias`},
                {n:3,desc:`Entrada ${fmt(wizTAd*0.5)} + ${fmt(wizTAd*0.25)} em 30 dias + ${fmt(wizTAd*0.25)} em 60 dias`}
              ].map(o=>(
...
const CSS=`
  ...
  *{box-sizing:border-box;margin:0;padding:0}html{font-size:15px}
  ...
  header{position:relative;z-index:10;width:100%;max-width:860px;padding:14px 20px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;background:rgba(10,15,30,.6);backdrop-filter:blur(12px);border-bottom:1px solid var(--border);position:sticky;top:0}
