...
export default async function handler(req, res) {
  ...
  if (cnaes.length === 0) {
    return res.status(200).json({
      resultados:[], cnaes:[],
      aviso:`Nicho "${nicho}" não mapeado. Use o campo CNAE para inserir o código diretamente, ou tente: contabilidade, advocacia, dentista, restaurante, farmácia, academia, clínica, imobiliária, escola, petshop, salão, barbearia, mecânica, hotel, tecnologia, psicólogo, nutricionista, engenharia, segurança, limpeza, marketing, consultoria, padaria, ótica, laboratório, turismo, seguro, supermercado, fisioterapia, construtora, gráfica, fotografia, logística...`,
    })
  }
    ...
    while (continua) {
      ...
      try {
        ...
        for (const emp of lista) {
          ...
          // ── Filtros ──
          const sit = (emp.situacao_cadastral||'').toLowerCase()
          if (situacaoFiltro==='ativa'   && !sit.includes('ativa'))  continue
          if (situacaoFiltro==='inativa' &&  sit.includes('ativa'))  continue
          ...
          // Enriquecer se faltar telefone ou email
          if (!lead.telefone || !lead.email) {
            await sleep(22) // ~45/s dentro do limite 50/s
  ...
  const aviso = semResultado ? [
    ...
    municipioCod ? `• Código IBGE usado: ${municipioCod}` : cidade ? `• Cidade "${cidade}" não encontrada no mapa IBGE — tente deixar o campo vazio` : null,
    ...
    '',
    'Se o problema persistir, acesse /api/leads/debug para ver quais APIs estão acessíveis.',
